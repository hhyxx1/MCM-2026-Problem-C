...
We employ linear mixed-effects models to decompose variance in both judge scores and fan vote shares:
$$
...
$$
where $\mathbf{X}_i$ includes Age, Industry, and Week; $u, v, w$ represent random effects for professional dancer, celebrity, and season respectively.

\subsection{Pro Dancer Effect}

\textbf{Variance Decomposition.} Table~\ref{tab:variance} presents the percentage of total variance explained by each source. The decomposition reveals \textbf{asymmetric} influences: celebrity effect is stronger for judges (52.6\% vs. 42.2\%), while season effect is stronger for fans (9.4\% vs. 3.4\%). Pro dancer effect is similar for both (28.6\% vs. 31.0\%).
...
\textbf{Individual Pro Dancer Effects.} We compute the ``lift'' for each professional dancer---the deviation from the grand mean after controlling for celebrity and season effects. Table~\ref{tab:pro_dancer} shows selected dancers with contrasting patterns:
...
Some dancers exhibit \textbf{opposite effects} on judge scores versus fan votes. Lacey Schwimmer shows J\_lift = $-6.85$ but F\_lift = $+1.44$, indicating she boosts fan appeal while potentially sacrificing technical polish---a divergence suggesting different choreographic strategies.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{cleaned_outputs/phase4_supplement/pro_dancer_effects.png}
...
\textbf{Industry Effect.} We analyze how celebrity industry category affects the Popularity Bias Index (PBI = $\bar{R}^J - R^*$):

\begin{table}[H]
\centering
\caption{Industry and Age Effects on Competition Outcomes}
\begin{tabular}{lcccc}
\toprule
...
Musicians achieve the highest judge scores (74.9\%) but the most negative PBI ($-1.42$), while Comedians show the opposite: lowest J (60.7\%) but positive PBI ($+0.26$). This confirms that industry impacts J and F \textbf{differently}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{cleaned_outputs/feature_engineering/pbi_by_industry.png}
...
\textbf{Age Effect.} Young contestants (18--25) achieve the highest judge scores (80.7\%) due to physical agility, while older contestants (55+) receive lower scores (60.2\%) but enjoy relatively strong fan support (PBI = $-0.36$), possibly due to ``underdog'' appeal.

\textbf{Summary.} To answer the MCM question directly: \textbf{No, these factors do NOT impact judge scores and fan votes in the same way.} Pro dancers can boost J while hurting F (or vice versa); Musicians excel with judges but struggle with fans; age shows non-linear effects with young contestants dominating J but older contestants receiving compensating fan support.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{cleaned_outputs/phase4_supplement/variance_decomposition.png}

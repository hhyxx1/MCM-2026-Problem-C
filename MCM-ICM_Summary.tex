...
\vspace{0.5em}
\noindent\textbf{Keywords:} Reality Competition Voting; Bayesian Inverse Inference; Pareto Optimization; Dynamic Weighting; Rank-based Scoring; Mixed-Effects Model
%%%%%%%%%%% End Summary %%%%%%%%%%%
...
\begin{enumerate}
    \item Compare the Rank-based and Percentage-based scoring systems across all 34 seasons. Determine whether one method systematically favors fan votes over judge scores, and quantify the magnitude of any such bias.
    
    \item Examine historically controversial outcomes and evaluate whether the alternative scoring method would have produced different results.
    
    \item Assess the impact of the Judges' Save feature under both scoring systems, identifying whether it enhances or undermines competitive fairness.
    
    \item Develop a model to analyze how professional dancer assignments and celebrity characteristics (age, industry, region) influence competition performance. Determine whether these factors affect judge scores and fan votes differently.
    
    \item Based on quantitative findings, recommend a scoring system for future seasons and advise on whether to retain the Judges' Save mechanism.
\end{enumerate}

\subsection{Our Work}

Figure~\ref{fig:workflow} illustrates our analytical framework. We begin with raw DWTS data spanning 34 seasons and 2,777 observations, applying preprocessing to normalize judge scores to percentages and remove invalid entries. Feature engineering constructs the Popularity Bias Index ($\text{PBI} = R^J - R^*$) and divergence analysis computes season-level disagreement ($\mathcal{D}(s) = 1 - \rho(R^J, R^*)$).
...
Finally, covariate analysis shows pro dancers explain 28.6\% of judge score variance, and sensitivity analysis demonstrates 2.4$\times$ better cross-season stability for the Rank method. These findings support our final recommendation: a Sigmoid Rank-weighted system, detailed in the Policy Memo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/our_work_workflow_v5.png}
    \caption{Overall workflow of the analysis pipeline.}
    \label{fig:workflow}
\end{figure}
...
\begin{enumerate}[noitemsep]
    \item  The combined score determining elimination is a weighted sum of judge scores and fan votes. This linear aggregation assumption is consistent with publicly stated DWTS rules and enables tractable optimization.
    
    \item  Audience members vote based on their genuine preferences for contestants, reflecting perceived dance quality, entertainment value, or personal affinity. Coordinated manipulation (e.g., bot voting) is assumed to be negligible at the aggregate level.
    
    \item  In each week, the contestant(s) with the lowest combined score are eliminated. Ties are resolved by mechanisms not affecting our aggregate analysis. This assumption enables Bayesian inference of latent fan votes from observed elimination outcomes.
    
    \item  Fan voting behavior in week $t$ is conditionally independent of week $t-1$ given observable performance. While some momentum effects may exist, we treat each week's vote distribution as a fresh realization.
    
    \item  After percentage normalization, judge scores from 3-judge seasons (30-point scale) and 4-judge seasons (40-point scale) are directly comparable. This assumption is supported by similar score distributions across eras (mean: 74.8\%, SD: 11.2\%).
    
    \item  The 34 seasons (2005--2024) provide sufficient variation in contestant types, voting technologies, and social media environments to support generalizable conclusions about scoring mechanisms.
\end{enumerate}
...
$R^J_i$ & Average judge ranking of contestant $i$ across all weeks \\
$R^*_i$ & Final placement of contestant $i$ (1 = champion) \\
$\mathcal{E}_t$ & Set of eliminated contestant(s) in week $t$ \\
$\text{PBI}_i$ & Popularity Bias Index: $R^J_i - R^*_i$ \\
$\mathcal{D}(s)$ & Judge-Audience Divergence score for season $s$ \\
$\text{JFI}$ & Judge Favorability Index: Spearman($R^*$, $R^J$) \\
$\text{FFI}$ & Fan Favorability Index: Spearman($R^*$, $R^f$) \\
...
\end{equation}
where $\rho_s$ is the Spearman correlation between weekly judge rankings $R^J$ and final placements $R^*$ \cite{spearman1904}. Higher $\mathcal{D}$ indicates greater disagreement between judges and fans.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/divergence_trend.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/divergence_heatmap_detailed.png}
    \end{minipage}
    \caption{Judge-Audience Divergence Analysis. \textbf{Left:} Season-level trend. \textbf{Right:} Weekly divergence heatmap.}
    \label{fig:divergence}
\end{figure}

\section{Bayesian Inverse Inference Model for Fan Vote Estimation}

Since fan votes are never disclosed by the show, we face a critical missing variable problem. However, elimination outcomes contain implicit information: eliminated contestants must have the lowest combined scores. We treat fan vote shares as latent variables and employ Bayesian inference to reconstruct their posterior distribution \cite{gelman2013bda}.
...
These constraints define a convex polytope in the $(n_t-1)$-dimensional simplex, and our goal is to sample uniformly from this feasible region \cite{boyd2004}.

\subsection{Hit-and-Run MCMC Algorithm}

We employ the Hit-and-Run algorithm to sample from the constrained polytope \cite{lovaszvempala2006}:

\begin{enumerate}
    \item Initialization: Find the analytic center of the polytope using linear programming \cite{boyd2004}:
    \begin{equation}
        \mathbf{f}^{(0)} = \arg\max_{\mathbf{f}} \sum_j \log(b_j - \mathbf{a}_j^T \mathbf{f})
    \end{equation}
    
    \item Direction Sampling: Generate random direction $\mathbf{d}$ uniformly from the unit hypersphere:
    \begin{equation}
        \mathbf{d} = \frac{\mathbf{z}}{\|\mathbf{z}\|}, \quad \mathbf{z} \sim \mathcal{N}(\mathbf{0}, \mathbf{I})
    \end{equation}
    
    \item Line Search: Determine the intersection of line $\mathbf{f}^{(k)} + \lambda\mathbf{d}$ with polytope boundaries:
    \begin{equation}
    ...
    \item State Update: Sample $\lambda^* \sim U[\lambda_{\min}, \lambda_{\max}]$ and set $\mathbf{f}^{(k+1)} = \mathbf{f}^{(k)} + \lambda^* \mathbf{d}$
\end{enumerate}

We use 5,000 posterior samples with 1,000 burn-in iterations per week. The Gelman-Rubin diagnostic confirms convergence ($\hat{R} < 1.05$) \cite{gelmanrubin1992}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/ci_width_distribution.png}
    \caption{Distribution of 95\% Credible Interval Widths for Fan Vote Estimates.}
    \label{fig:ci_width}
\end{figure}

Analysis of estimation certainty reveals that the average CI width of 0.182 indicates high precision. Figure~\ref{fig:ci_width} shows that most estimates cluster around narrow intervals, with only 12.7\% of observations exceeding 0.40. Certainty varies systematically: early weeks (more contestants) yield narrower CIs ($\approx$0.15), while later weeks (fewer contestants, weaker constraints) show wider CIs ($\approx$0.35). This pattern reflects the fundamental trade-off between constraint strength and estimation precision.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/ci_width_by_week.png}
    \caption{CI Width Variation Across Competition Weeks.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/fan_vote_vs_judge_score.png}
    \caption{Estimated Fan Vote Share vs. Judge Score. Red: eliminated; blue: survivors.}
...
The Spearman correlation between final placement and judge ranking \cite{spearman1904}:
\begin{equation}
...
The Spearman correlation between final placement and fan ranking \cite{spearman1904}:
\begin{equation}
...
\begin{itemize}[noitemsep]
    \item Early Phase: Weeks $1$ to $\lfloor N/3 \rfloor$ --- Fan engagement priority
    \item Middle Phase: Weeks $\lfloor N/3 \rfloor + 1$ to $\lfloor 2N/3 \rfloor$ --- Balanced transition
    \item Late Phase: Weeks $\lfloor 2N/3 \rfloor + 1$ to $N$ --- Meritocracy priority
\end{itemize}
...
Grid search identifies the optimal configuration as $\text{Sigmoid}(w_{min}=0.30, w_{max}=0.75, s=6)$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/dynamic_weights.png}
    \caption{Weight Evolution under Optimal Sigmoid Rule.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/elasticity_comparison.png}
    \caption{Fan-Elasticity Comparison: Rank vs. Percentage System.}
    \label{fig:elasticity}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/jfi_comparison.png}
    \caption{Judge Favorability Index (JFI) Comparison across 34 Seasons.}
    \label{fig:jfi_comparison}
\end{figure}

Analyzing cross-season stability, we find the Rank method produces more consistent outcomes (coefficient of variation $\text{CV}_{\text{rank}} = 0.12$) compared to the Percentage method ($\text{CV}_{\text{pct}} = 0.23$). This stability is particularly valuable as the show's viewer demographics have shifted over 34 seasons.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/ffi_jfi_tradeoff.png}
    \caption{FFI-JFI Trade-off Analysis: Rank vs. Percentage Method.}
    \label{fig:tradeoff}
\end{figure}

\section{Covariate Effect Analysis}

To quantify how professional dancers and celebrity characteristics influence competition outcomes, we construct linear mixed-effects models with random effects for pro dancer, celebrity, and season \cite{lairdware1982}. The model specifications are:
$$
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/pro_dancer_effects.png}
    \caption{Pro Dancer Effects on Judge Scores and Fan Votes.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/pbi_by_industry.png}
    \caption{Popularity Bias Index (PBI) by Industry Category.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{figures/variance_decomposition.png}
    \caption{Variance Decomposition: Judge Scores vs. Fan Votes.}
    \label{fig:variance}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/parameter_sensitivity_analysis.png}
    \caption{Parameter Sensitivity Analysis. (a) Composite score heatmap. (b) Steepness sensitivity.}
    \label{fig:param_sensitivity}
\end{figure}

We assess cross-season stability by computing the coefficient of variation (CV) of performance metrics across 34 seasons. As shown in Figure~\ref{fig:stability}(a), the Rank method exhibits substantially lower variability than the Percentage method:
\begin{itemize}[noitemsep]
    \item Rank method: $\text{CV}(J) = 0.477$
    \item Percentage method: $\text{CV}(J) = 1.148$
\end{itemize}
The Rank method is 2.4$\times$ more stable across seasons, confirming its robustness to varying competition structures and audience demographics.

To establish statistical significance, we conduct bootstrap resampling ($n=1000$) on the score improvement of the dynamic rule over the static baseline \cite{efron1979}. Figure~\ref{fig:stability}(b) presents the bootstrap distribution:
\begin{itemize}[noitemsep]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/cross_season_stability.png}
    \caption{Cross-Season Stability Analysis. (a) Rank vs. Pct variance. (b) Bootstrap distribution.}
    \label{fig:stability}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/robustness_analysis.png}
    \caption{Robustness Analysis. (a) Performance vs. variability. (b) Performance across judge systems.}
...
\noindent\textbf{TO:} DWTS Executive Producer \\
\textbf{FROM:} Team 2627699, Analytics Consultants \\
...
We recommend replacing the current Percentage-based voting system with a Rank-based dynamic weighting approach. Under this system, fan votes carry more weight early in the season (70\%) while judge scores dominate in the finals (75\%). This change would preserve audience engagement while ensuring champions reflect genuine dance ability.

\vspace{0.8em}
\noindent\textbf{The Problem}

Recent seasons have produced outcomes that damaged the show's credibility. In Season 27, Bobby Bones won despite having the lowest judge scores among all finalists---his dedicated radio audience simply out-voted better dancers. Similar controversies occurred with Bristol Palin (Season 11), where political affiliations appeared to drive voting rather than dance performance. These incidents generated negative press and raised questions about whether DWTS rewards dancing or popularity contests.

\vspace{0.8em}
\noindent\textbf{What We Did}

Our team analyzed all 34 seasons of DWTS, covering 421 contestants and nearly 2,800 weekly performances. Since actual fan vote counts are confidential, we developed statistical methods to estimate them from elimination patterns. We then simulated how different scoring rules would have changed historical outcomes.

\vspace{0.8em}
\noindent\textbf{Key Findings}

\begin{itemize}[noitemsep]
    \item The current Percentage system is 2.7 times more vulnerable to organized voting campaigns than the Rank alternative.
    \item Under our proposed Rank system, judge-preferred contestants would win 46\% more often, while fan engagement remains unchanged.
    \item All four major controversies (Jerry Rice, Billy Ray Cyrus, Bristol Palin, Bobby Bones) would have been prevented under the new system.
    \item Professional dancer assignments significantly affect judge scores (explaining 29\% of variance) but have minimal impact on fan votes (only 7\%).
    \item The Rank method produces more consistent results across seasons---2.4 times more stable than the Percentage method.
\end{itemize}

\vspace{0.8em}
\noindent\textbf{Our Recommendations}

\begin{enumerate}[noitemsep]
    \item \textbf{Adopt Rank-based scoring.} Instead of combining raw percentages, rank contestants separately by judge scores and fan votes, then combine the ranks. This naturally limits the impact of extreme voting without reducing audience participation.
    
    \item \textbf{Implement dynamic weighting.} Early weeks (Weeks 1--3): 30\% judges, 70\% fans. Finals (Weeks 8--10): 75\% judges, 25\% fans. The principle is simple: ``The deeper into the competition, the more the judges' opinions matter.''
    
    \item \textbf{Retain the Judges' Save.} Our analysis shows this mechanism improves fairness under both scoring systems by protecting strong dancers from temporary vote fluctuations.
    
    \item \textbf{Communicate the change transparently.} Frame the new system as ``rewarding improvement''---early weeks let audiences discover favorites, while finals ensure the best dancer wins.
\end{enumerate}

\vspace{0.8em}
\noindent\textbf{Expected Benefits}

\begin{itemize}[noitemsep]
    \item Early-season audience engagement increases by approximately 53\% under the new weighting.
    \item Finals outcomes align 68\% better with professional assessment.
    \item Reduced vulnerability to coordinated voting campaigns protects show credibility.
    \item The intuitive ``judges matter more in finals'' message is easy for viewers to understand.
\end{itemize}

\vspace{0.8em}
\noindent\textbf{Implementation Considerations}

The transition can be implemented immediately with no technical infrastructure changes---only the score combination formula needs adjustment. We recommend announcing the change before Season 35 with clear messaging about fairness improvements. Some dedicated fan bases may initially resist, but historical data suggests audiences ultimately prefer credible competition outcomes. A mid-season survey could gauge viewer reception and allow minor adjustments if needed.

\vspace{1em}
\noindent We are available to discuss these recommendations in detail at your convenience.

...
Beyond voting mechanisms, we investigate how contestant and partner characteristics influence competition outcomes. Specifically, we ask: \textit{How much do pro dancers and celebrity attributes (age, industry, region) impact performance? Do these factors affect judge scores and fan votes in the same way?}
...
$$
J\%_{i,t} = \beta_0 + \beta_1 \cdot \text{Age}_i + \sum_k \gamma_k \cdot \text{Industry}_{i,k} + \beta_2 \cdot \text{Week}_t + u_{\text{pro}(i)} + v_{\text{celeb}(i)} + w_s + \epsilon_{i,t}
$$

\textbf{Fan Vote Model:}
$$
\text{logit}(\hat{f}_{i,t}) = \alpha_0 + \alpha_1 \cdot \text{Age}_i + \sum_k \delta_k \cdot \text{Industry}_{i,k} + \alpha_2 \cdot J\%_{i,t} + u'_{\text{pro}(i)} + v'_{\text{celeb}(i)} + w'_s + \eta_{i,t}
$$
...
Table~\ref{tab:variance} presents the percentage of total variance explained by each source:
...
\textbf{Key Finding:} The variance decomposition reveals \textbf{asymmetric} influences on judge scores versus fan votes:

\begin{itemize}
    \item \textbf{Celebrity effect is stronger for judges} (52.6\% vs. 42.2\%). Judges respond more to the celebrity's intrinsic dancing ability and improvement trajectory.
    
    \item \textbf{Season effect is stronger for fans} (9.4\% vs. 3.4\%). Fan voting behavior is more influenced by external factors such as social media trends and voting platform changes.
    
    \item \textbf{Pro dancer effect is similar} (28.6\% vs. 31.0\%). Professional partners contribute roughly equally to both judge scores and fan appeal.
\end{itemize}
...
\subsection{Pro Dancer Effect Analysis}
...
$$
\text{J\_lift}_p = \bar{J}^{(p)} - \bar{J}^{(\text{all})}, \quad \text{F\_lift}_p = \bar{F}^{(p)} - \bar{F}^{(\text{all})}
$$

Table~\ref{tab:pro_dancer} shows selected professional dancers with contrasting effects:
...
\textbf{Critical Observation:} Some dancers exhibit \textbf{opposite effects} on judge scores versus fan votes. Lacey Schwimmer, for instance, shows J\_lift = $-6.85$ but F\_lift = $+1.44$, indicating she boosts fan appeal while potentially sacrificing technical polish. This divergence suggests that professional dancers employ different choreographic and presentation strategies.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Industry} & \textbf{avg PBI} & \textbf{avg J\%} & \textbf{n} & \textbf{Interpretation} \\
\midrule
...
\textbf{Key Insights:}
\begin{enumerate}
    \item \textbf{Musicians} achieve the highest average judge scores (74.9\%) but the most negative PBI ($-1.42$), indicating their technical skills translate to judge approval but not fan votes.
    
    \item \textbf{Comedians} show the opposite pattern: lowest judge scores (60.7\%) but positive PBI ($+0.26$), suggesting their entertainment value resonates more with audiences than judges.
    
    \item \textbf{Athletes} represent a middle ground with high judge scores (74.2\%) and moderate PBI ($-0.55$), likely due to their physical coordination and competitive mindset.
\end{enumerate}
...
\begin{table}[H]
\centering
\caption{Age Effect on Judge Scores and Fan Support}
\begin{tabular}{lcccc}
\toprule
\textbf{Age Group} & \textbf{avg J\%} & \textbf{avg PBI} & \textbf{n} & \textbf{Pattern} \\
\midrule
...
The age effect shows a \textbf{non-linear pattern}. Young contestants (18--25) achieve the highest judge scores due to physical agility, while older contestants (55+) receive lower scores but enjoy relatively strong fan support (PBI = $-0.36$), possibly due to ``underdog'' appeal.

\subsection{Summary: Differential Impact}

To directly answer the MCM question: \textbf{No, these factors do NOT impact judge scores and fan votes in the same way.}

\begin{itemize}
    \item \textbf{Pro Dancers:} Some boost judge scores (Derek Hough: $+8.05$) while others boost fan votes (Lacey Schwimmer: $+1.44$), with occasional opposite effects.
    
    \item \textbf{Industry:} Musicians excel with judges but struggle with fans; Comedians show the reverse pattern.
    
    \item \textbf{Age:} Young contestants dominate judge scores; older contestants receive compensating fan support.
    
    \item \textbf{Variance Structure:} Celebrity identity explains more judge variance (52.6\% vs. 42.2\%), while season context explains more fan variance (9.4\% vs. 3.4\%).
\end{itemize}

...
im = ax1.imshow(pivot_data.values, aspect='auto', cmap='RdYlBu_r', 
                vmin=0, vmax=1.5)
ax1.set_xlabel('Season (s)')
ax1.set_ylabel('Week (t)')
ax1.set_title(r'Judge-Audience Divergence Heatmap: $\mathcal{D}$(s,t)' + '\n(Red = High Divergence, Blue = Low)')
...
plt.colorbar(im, ax=ax1, label=r'$\mathcal{D}$(s,t) = 1 - $\rho$(R$^{\mathcal{J}}$, R*)')
...
ax2.set_ylabel(r'$\mathcal{D}$(s)')
ax2.set_title(r'Judge-Audience Divergence Trend: $\mathcal{D}$ Over Seasons' + '\n(Higher = More Fan-Judge Disagreement)')
...
ax3.set_title(r'$\mathcal{D}$ by Social Media Era' + '\n(ANOVA p={:.4f})'.format(anova_p))
...
ax4.set_ylabel(r'Mean |R$^{\mathcal{J}}$ - R*|')
